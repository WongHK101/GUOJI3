...
def main():
    ...
    for seed in range(3):
        p = f"/root/autodl-tmp/GUOJI/mamba_enhanced/data/nonstationary_var_planA/num_nodes_50/true_lag_14/noise_scale_1/seed_{seed}"
    ...
    p = "/root/autodl-tmp/GUOJI/JRNGC/data/causaltime/traffic"
    x = np.load(os.path.join(p, "_x.npy"))
    gc = np.load(os.path.join(p, "_gc.npy"))
    d = x.shape[0]
    ...
    fmri_base = "/root/autodl-tmp/GUOJI/JRNGC/data/fmri/num_nodes_15"
    for subj in range(3):
