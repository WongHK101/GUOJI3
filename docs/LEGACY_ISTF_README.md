> **Historical document.** This README records the superseded ISTF-Mamba
> performance-method phase of the project. Its method-success, benchmark, and
> operating-regime claims are not the current scientific position and must not
> be used to describe the active KBS manuscript. See the repository-root
> `README.md` for the current Jacobian-coverage audit project.
