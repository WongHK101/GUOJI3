...
def main():
    ...
    for seed in range(3):
        p = os.path.join(JRNGC_DATA, "var", "num_nodes_50", "true_lag_5", "noise_scale_1", f"seed_{seed}")
    ...
    results["Lorenz_F40"] = {}
    for seed in range(3):
        p = os.path.join(JRNGC_DATA, "lorenz", "num_nodes_10", "F_40", f"seed_{seed}")
        x = np.load(os.path.join(p, "_x.npy"))
        gc = np.load(os.path.join(p, "_gc.npy"))
        t0 = time.time()
    ...
    log("=" * 60)
    p = os.path.join(JRNGC_DATA, "lorenz", "num_nodes_10", "F_40", "seed_0")
    x = np.load(os.path.join(p, "_x.npy"))
    gc = np.load(os.path.join(p, "_gc.npy"))
