"""5-seed verification of Mamba filter with variance fixes (ortho_lam=0.05, residual_scale=0.1).

Compares: A_baseline, F_ds8_OLD, F_ds8_fixAB05
Goal: Confirm fix narrows cross-seed AUROC std from 0.047 to ≤0.030.
"""
...
# Summary
print(f"\n{'='*60}")
print(f"SUMMARY (5 seeds) — Variance Fix Verification")
print(f"{'='*60}")
for name in ["A_baseline", "F_ds8_OLD", "F_ds8_fix"]:
    ...
    # Per-seed vs baseline delta
    bl_aurocs = [results[f"seed_{s}"]["A_baseline"]["auroc"] for s in range(5)]
    deltas = [aurocs[s] - bl_aurocs[s] for s in range(5)]
    print(f"  Δ vs baseline: ", end="")
    for s in range(5):
        print(f"{deltas[s]:+.4f} ", end="")
    print()

with open("filter_5seed_fix_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("\nSaved to filter_5seed_fix_results.json")
