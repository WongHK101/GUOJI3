"""Release-locked Phase 9 Stage B held-out confirmation runner."""
...
PRIMARY_ROLES = {"conditional_external_confirmation"}
...
def validate_release_lock(
    *,
    ...
    matrix_path: Path,
    token_path: Path,
    stage_a_decision_path: Path,
) -> Dict[str, object]:
    token = load_json(token_path)
    if token.get("protocol_name") != "phase9_audit_generality_stageb_v1":
        raise RuntimeError("Release token protocol name mismatch")
    if token.get("authorized_stage") != STAGE:
        raise RuntimeError("Release token does not authorize Stage B")
    if token.get("execution_authorized") is not True:
        raise RuntimeError("Release token is not authorized")
    if token.get("stage_b_authorized") is not True:
        raise RuntimeError("Release token does not authorize Stage B")
    if token.get("autodl_authorized") is not True:
        raise RuntimeError("Release token does not authorize AutoDL")
    config = load_json(config_path)
    stage_a_decision_sha = canonical_text_sha256(stage_a_decision_path)
    if stage_a_decision_sha != token.get(
        "stage_a_decision_canonical_sha256"
    ):
        raise RuntimeError("Stage A decision canonical SHA256 mismatch")
    ...
    if (
        config.get("required_stage_a_decision_canonical_sha256")
        != stage_a_decision_sha
    ):
        raise RuntimeError("Config Stage A decision canonical SHA256 mismatch")
    parent_matrix_path = (
        PROJECT_ROOT
        / "paper-data"
        / "docs"
    ...
    parent_matrix_sha = canonical_text_sha256(parent_matrix_path)
    if (
        config.get("parent_preregistered_matrix_sha256")
        != parent_matrix_sha
        or token.get("parent_preregistered_matrix_sha256")
        != parent_matrix_sha
    ):
    ...
    if worktree_status:
        raise RuntimeError("Stage B requires a clean Git worktree")
    ...
    if config_sha != token.get("config_sha256"):
        raise RuntimeError("Stage B config SHA256 mismatch")
    if matrix_sha != token.get("matrix_sha256"):
        raise RuntimeError("Stage B matrix SHA256 mismatch")
    ...
    return {
        ...
        "stage_a_decision_canonical_sha256": stage_a_decision_sha,
...
def validate_matrix(
    ...
    if len(selected) != 36:
        raise RuntimeError(f"Expected 36 Stage B records, found {len(selected)}")
    ...
    if {row["method"] for row in selected} != METHODS:
        raise RuntimeError("Stage B method set mismatch")
    units = set(config["datasets"])  # type: ignore[arg-type]
    if {row["data_unit"] for row in selected} != units:
        raise RuntimeError("Stage B data-unit set mismatch")
    combinations = {
        (row["data_unit"], row["method"], int(row["replicate"]))
        for row in selected
    }
    if len(combinations) != 36:
        raise RuntimeError("Stage B unit/method/replicate matrix mismatch")
    for row in selected:
        if row["execution_authorized"] != "true":
            raise RuntimeError(f"Unauthorized Stage B row: {row['run_id']}")
        if row["environment"] != "autodl_frozen_confirmation":
            raise RuntimeError(f"Unexpected Stage B environment: {row['run_id']}")
        if row["method"] not in METHODS:
            raise RuntimeError(f"Unexpected method: {row['run_id']}")
        if row["duplicate_of"]:
            raise RuntimeError(f"Stage B cannot contain duplicates: {row['run_id']}")
        if int(row["replicate"]) not in {1, 2}:
            raise RuntimeError(f"Unexpected Stage B replicate: {row['run_id']}")
...
def validate_smoke_gate(
    ...
    *,
    ...
    if payload.get("passed") is not True:
        raise RuntimeError("Stage B smoke validation did not pass")
    if payload.get("formal_scientific_evidence") is not False:
        raise RuntimeError("Smoke validation evidence label mismatch")
    expected_fingerprint = release_lock_fingerprint(release_lock)
    if payload.get("release_lock_fingerprint") != expected_fingerprint:
        raise RuntimeError("Smoke validation release lock mismatch")
    if payload.get("approved_commit") != release_lock.get("approved_commit"):
        raise RuntimeError("Smoke validation commit mismatch")
    if (
        payload.get("release_token_sha256")
        != release_lock.get("release_token_sha256")
    ):
        raise RuntimeError("Smoke validation token mismatch")
    if (
        payload.get("stage_a_decision_canonical_sha256")
        != release_lock.get("stage_a_decision_canonical_sha256")
    ):
        raise RuntimeError("Smoke validation Stage A decision mismatch")
    return {
        "sha256": file_sha256(path),
        "release_lock_fingerprint": expected_fingerprint,
    }
...
def run_record(
    ...
    *,
    ...
    run_id = row["run_id"]
    formal_run = smoke_iterations is None
    ...
    atomic_json(
        status_path,
        {
            "status": "running",
            "run_id": run_id,
            "validation_only": not formal_run,
            "formal_result": formal_run,
            "confirmation_candidate": formal_run,
            "manuscript_evidence": False,
        },
    )
    ...
    profile.update(
        {
            "formal_result": formal_run,
            "development_only": not formal_run,
            "confirmation_candidate": formal_run,
            "manuscript_evidence": False,
        }
    )
...
def main() -> int:
    ...
    if config.get("stage") != STAGE:
        raise RuntimeError("Config is not Stage B")
    ...
    smoke_gate = None
    if args.smoke_iterations is None:
        if args.smoke_validation is None:
            raise RuntimeError(
                "Formal Stage B requires a passed smoke-validation file"
            )
        smoke_gate = validate_smoke_gate(
            args.smoke_validation.resolve(),
            release_lock=release_lock,
        )
    elif args.smoke_validation is not None:
        raise RuntimeError("Smoke execution must not consume a smoke gate")
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("Stage B execution requires CUDA")
    ...
    if not any(args.output_root.iterdir()):
        ...
        shutil.copy2(
            stage_a_decision_path,
            args.output_root / "stage_a_decision_snapshot.json",
        )
        if smoke_gate is not None:
            shutil.copy2(
                args.smoke_validation.resolve(),
                args.output_root / "smoke_validation_snapshot.json",
            )
        ...
        if smoke_gate is not None:
            stored_smoke = args.output_root / "smoke_validation_snapshot.json"
            if (
                not stored_smoke.is_file()
                or file_sha256(stored_smoke) != smoke_gate["sha256"]
            ):
                raise RuntimeError("Output root smoke-validation mismatch")
    statuses = [
        run_record(
            row,
            config=config,
    ...
    atomic_json(
        args.output_root / "execution_summary.json",
        {
            ...
            "validation_only": args.smoke_iterations is not None,
            "formal_result": args.smoke_iterations is None,
            "confirmation_candidate": args.smoke_iterations is None,
            "manuscript_evidence": False,
