"""Aggregate and mechanically gate Phase 9 Stage A audit validation."""
...
def read_run(root: Path, row: Mapping[str, str]) -> Dict[str, object]:
    ...
    if status.get("status") != "complete" or status.get("smoke"):
        raise RuntimeError(f"Run is not a complete primary record: {row['run_id']}")
...
def unit_records(
    ...
    for run in runs:
        if (
            run["evidence_role"] == "internal_prospective_go_no_go"
...
def report_markdown(result: Mapping[str, object]) -> str:
    lines = [
        "# Phase 9 Audit-Generality Stage A Decision",
        "",
        ...
        f"- determinism: `{result['determinism']['passed']}`",
    ...
    lines.extend(
        [
            "## Evidence boundary",
            "",
            "- Stage A is internal prospective go/no-go evidence.",
            "- No Stage A value is manuscript confirmation evidence.",
            "- MoCap has no accepted direct graph ground truth.",
            "- Attribution beyond H=128 remains unassessed.",
            "- AutoDL Stage B remains closed unless the decision is "
            "`UNLOCK_STAGE_B`.",
            "",
        ]
    )
...
def main() -> int:
    ...
    config = load_json(root / "config_snapshot.json")
    matrix = read_matrix(root / "matrix_snapshot.csv")
    expected_rows = [
        row
        for row in matrix
        if row["stage"] == STAGE
    ...
    if len(expected_rows) != 56:
        raise RuntimeError("Frozen Stage A matrix cardinality mismatch")
    runs = [read_run(root, row) for row in expected_rows]
    primary = [
        run
        for run in runs
        if run["evidence_role"] == "internal_prospective_go_no_go"
    ]
    if len(primary) != 54:
        raise RuntimeError("Expected 54 primary Stage A records")
    gates = config["gates"]
    ...
    predictor_parity = {
        "passed": len(predictor_comparisons) == 18
    ...
    determinism = determinism_audit(
        root,
        runs,
        float(gates["determinism_max_abs"]),
    )
    semantic_checks = {
        "finite": finite_pass,
    ...
    runtime_pass = gpu_hours <= float(gates["rtx4090_gpu_hour_hard_cap"])
    unlock = (
        semantic_passed
        and runtime_pass
    ...
    decision = (
        ...
        if unlock
            ...
            else "STAGE_A_GENERALITY_GATE_FAIL"
        )
    )
    result = {
        "decision": decision,
        "unlock_stage_b": unlock,
        ...
        "duplicate_run_count": len(runs) - len(primary),
        "stage_a_is_manuscript_confirmation": False,
        "mass_beyond_H128_assessed": False,
    }
    atomic_json(root / "stagea_gate_decision.json", result)
    atomic_text(root / "STAGEA_DECISION.md", report_markdown(result))
