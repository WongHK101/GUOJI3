...
def main():
    ...
    jrngc_data = resolve_data_dir()
    our_data = os.path.join(_PROJ_ROOT, "data")

    configs = [
        {
            "name": "VAR_d50",
            "path": os.path.join(jrngc_data, "var", "num_nodes_50", "true_lag_5", "noise_scale_1"),
            "n_seeds": 5, "lag": 5, "d": 50,
        },
        {
            "name": "Lorenz_F40",
            "path": os.path.join(jrngc_data, "lorenz", "num_nodes_10", "F_40"),
            "n_seeds": 5, "lag": 1, "d": 10,
        },
        {
            "name": "NSVAR_d10",
            "path": os.path.join(our_data, "nonstationary_var", "num_nodes_10", "true_lag_7", "noise_scale_1"),
            "n_seeds": 5, "lag": 7, "d": 10,
        },
        {
            "name": "NSVAR_d50_PlanA",
            "path": os.path.join(our_data, "nonstationary_var_planA", "num_nodes_50", "true_lag_14", "noise_scale_1"),
    ...
    out_path = os.path.join(results_dir, "multiseed_synthetic_results.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Multi-seed synthetic validation")
