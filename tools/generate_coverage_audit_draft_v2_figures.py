"""Render draft v2 figures for the Jacobian coverage audit manuscript.
...
DEFAULT_OUTPUT_DIR = (
    WORKSPACE_ROOT / "elsarticle" / "figures" / "coverage_audit_draft_v2"
)
...
def draw_coverage_mismatch(output_dir: Path) -> list[Path]:
    ...
    ax.text(5, 9.25, "Unscored auxiliary route", ha="center", va="center", fontsize=8.5,
            fontweight="bold")
    ...
    ledger = [
        (r"$r_x$: direct history", "scored", "penalized", PALETTE["blue_soft"], PALETTE["blue"]),
        (r"$r_c$: auxiliary channel", "not-scored", "unpenalized", PALETTE["red_soft"], PALETTE["red"]),
    ]
    ax.text(1.0, 6.45, "route class", fontsize=5.6, fontweight="bold", ha="left")
    ax.text(5.55, 6.45, "score", fontsize=5.6, fontweight="bold", ha="center")
    ax.text(8.05, 6.45, "penalty", fontsize=5.6, fontweight="bold", ha="center")
    for idx, (route, score, penalty, face, edge) in enumerate(ledger):
        y = 5.4 - idx * 1.65
        rounded_box(ax, 0.45, y, 4.05, 0.95, route, face=face, edge=edge, fontsize=5.5, weight="bold")
        rounded_box(ax, 4.7, y, 1.85, 0.95, score, face="#F7F7F7", edge=edge, fontsize=4.5)
        rounded_box(ax, 6.85, y, 2.65, 0.95, penalty, face="#F7F7F7", edge=edge, fontsize=4.5)
    rounded_box(ax, 0.9, 1.15, 8.2, 1.1, "The ledger records status for declared route classes,\nnot every activated neural path.",
                face=PALETTE["gray_soft"], edge=PALETTE["gray"], fontsize=5.8)
    ...
    ax.text(5, 9.25, "Claim-specific audit", ha="center", va="center", fontsize=8.5, fontweight="bold")
    outcomes = [
    ...
    for (label, face, edge), y in zip(outcomes, ys):
        rounded_box(ax, 1.25, y, 7.5, 1.05, label, face=face, edge=edge, fontsize=6.1, weight="bold")
    return save_figure(fig, output_dir, "fig1_coverage_mismatch_draft_v2")
...
def draw_controlled_diagnostics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig2_controlled_concat_diagnostics_draft_v2")
...
def draw_score_semantics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig3_score_semantics_audit_draft_v2")
...
def collect_data() -> tuple[dict, dict]:
    ...
    manifest = {
        "purpose": "Frozen source inputs used by the draft v2 figures.",
        "sources": source_rows,
        "notes": {
...
def main() -> None:
    ...
    with (args.output_dir / "figure_data_manifest_draft_v2.json").open("w", encoding="utf-8") as handle:
