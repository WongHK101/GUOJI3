"""Render submission-candidate v2.4 figures for the coverage-audit manuscript.
...
def draw_coverage_mismatch(output_dir: Path) -> list[Path]:
    ...
    fig = plt.figure(figsize=(7.25, 3.40))
    grid = fig.add_gridspec(1, 3, width_ratios=[1.32, 1.05, 0.90], wspace=0.18)
    ...
    ax.text(5, 9.25, "Verified controlled instantiation", ha="center", va="center", fontsize=8.5,
            fontweight="bold")
    rounded_box(ax, 0.55, 6.80, 2.25, 1.05, "raw prefix\n" + r"$X_{0:t-1}$", face=PALETTE["blue_soft"], edge=PALETTE["blue"], fontsize=6.5, weight="bold")
    rounded_box(ax, 0.40, 3.75, 2.60, 1.30, r"$g_\phi$" + "\ncausal encoder", face=PALETTE["red_soft"], edge=PALETTE["red"], fontsize=6.1, weight="bold")
    rounded_box(ax, 3.45, 3.88, 1.85, 1.05, r"$c=g_\phi(X)$", face=PALETTE["red_soft"], edge=PALETTE["red"], fontsize=6.3, weight="bold")
    rounded_box(ax, 3.95, 6.50, 2.45, 1.40, "concat MLP", face="#F7F7F7", edge=PALETTE["ink"], fontsize=6.5, weight="bold")
    rounded_box(ax, 7.30, 6.65, 1.90, 1.10, "prediction\n" + r"$\hat{x}_t$", face=PALETTE["teal_soft"], edge=PALETTE["teal"], fontsize=6.8, weight="bold")
    orthogonal_arrow(ax, (2.80, 7.32), (3.95, 7.32), PALETTE["blue"])
    orthogonal_arrow(ax, (1.68, 6.80), (1.68, 5.05), PALETTE["red"])
    orthogonal_arrow(ax, (2.80, 4.42), (3.45, 4.42), PALETTE["red"])
    ax.plot([5.30, 5.30, 5.10], [4.42, 5.70, 5.70], color=PALETTE["red"], lw=1.5, ls=(0, (3, 2)))
    orthogonal_arrow(ax, (5.10, 5.70), (5.10, 6.50), PALETTE["red"], linestyle=(0, (3, 2)))
    orthogonal_arrow(ax, (6.40, 7.20), (7.30, 7.20), PALETTE["ink"])
    rounded_box(ax, 0.85, 1.15, 8.30, 1.50, r"x-only score and penalty: $S_x, R_x$" + "\n" + r"$\partial\hat{x}/\partial X$ with $c$ fixed" + "\nauxiliary chain route omitted", face=PALETTE["gray_soft"], edge=PALETTE["gray"], fontsize=5.5, weight="bold")
    ...
    ax.text(5, 9.25, "Architecture route ledger", ha="center", va="center", fontsize=8.5,
            fontweight="bold")
    rounded_box(ax, 0.55, 7.35, 8.9, 1.00, r"$C=(V_{\rm score},V_{\rm penalty},P_{\rm pred},M_{\rm coord},H_{\rm attr})$",
                face="#F7F7F7", edge=PALETTE["ink"], fontsize=5.5, weight="bold")
    ledger = [
        (r"history $X$", "yes", "yes", "no", PALETTE["blue_soft"], PALETTE["blue"]),
        (r"auxiliary $c$", "no", "no", "no", PALETTE["red_soft"], PALETTE["red"]),
    ]
    ax.text(0.65, 6.38, "route", fontsize=5.5, fontweight="bold", ha="left")
    ax.text(4.58, 6.38, "score", fontsize=5.2, fontweight="bold", ha="center")
    ax.text(6.42, 6.38, "penalty", fontsize=5.2, fontweight="bold", ha="center")
    ax.text(8.58, 6.38, "exempt", fontsize=5.2, fontweight="bold", ha="center")
    for idx, (route, score, penalty, exemption, face, edge) in enumerate(ledger):
        y = 5.35 - idx * 1.65
        rounded_box(ax, 0.35, y, 3.35, 0.95, route, face=face, edge=edge, fontsize=5.4, weight="bold")
        rounded_box(ax, 3.85, y, 1.55, 0.95, score, face="#F7F7F7", edge=edge, fontsize=5.4, weight="bold")
        rounded_box(ax, 5.55, y, 1.85, 0.95, penalty, face="#F7F7F7", edge=edge, fontsize=5.4, weight="bold")
        rounded_box(ax, 7.55, y, 2.10, 0.95, exemption, face="#F7F7F7", edge=edge, fontsize=5.4, weight="bold")
    rounded_box(
        ax,
        0.9,
        1.05,
    ...
    return save_figure(fig, output_dir, "fig1_coverage_mismatch_submission_v2_4")
...
def draw_controlled_diagnostics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig2_controlled_concat_diagnostics_submission_v2_4")
...
def draw_score_semantics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig3_score_semantics_audit_submission_v2_4")
...
def collect_data() -> tuple[dict, dict]:
    project_sources = {
        ...
        "posthoc": PROJECT_ROOT / "risk_mitigation_results" / "concat_posthoc_jacobian.json",
        "p0_audits": [
    ...
    package_sources = {
        ...
        "posthoc": frozen_root / "concat_posthoc" / "concat_posthoc_jacobian.json",
        "p0_audits": [
    ...
    manifest = {
        "purpose": "Frozen source inputs used by the v2.4 submission-candidate figures.",
        "sources": source_rows,
        "notes": {
            "fig1": "Source-verified controlled concat instantiation: c=g_phi(X) is generated by a causal full-prefix encoder; x-only scoring and penalty hold c fixed. The schematic is not a universal statement about auxiliary conditioning.",
            "fig2": "Frozen controlled concat diagnostics. Panels a-c are explicitly single-run diagnostics; panel d is a selected-endpoint view of the five-seed penalty experiment. All frozen penalty variants are disclosed in Appendix Table B.2. The no-aux intervention comparator only contextualizes route sensitivity.",
            ...
            "posthoc_jacobian_ratio": "The frozen |J_c|/|J_x|=3.689 +/- 0.750 quantity is a ratio of blockwise mean absolute Jacobian magnitudes computed with torch.mean. It is coordinate-scale dependent and is not a total-mass measure, graph score, causal contribution, Granger strength, benchmark, or confirmatory quantity.",
        },
    }
...
def main() -> None:
    ...
    with (args.output_dir / "figure_data_manifest_submission_v2_4.json").open("w", encoding="utf-8") as handle:
