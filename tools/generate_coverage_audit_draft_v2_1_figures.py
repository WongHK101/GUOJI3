"""Render draft v2.1 figures for the Jacobian coverage audit manuscript.
...
DEFAULT_OUTPUT_DIR = (
    WORKSPACE_ROOT / "elsarticle" / "figures" / "coverage_audit_draft_v2_1"
)
...
def draw_coverage_mismatch(output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig1_coverage_mismatch_draft_v2_1")
...
def draw_controlled_diagnostics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig2_controlled_concat_diagnostics_draft_v2_1")
...
def draw_score_semantics(data: dict, output_dir: Path) -> list[Path]:
    ...
    return save_figure(fig, output_dir, "fig3_score_semantics_audit_draft_v2_1")
...
def collect_data() -> tuple[dict, dict]:
    ...
    manifest = {
        "purpose": "Frozen source inputs used by the draft v2.1 figures.",
        "sources": source_rows,
        "notes": {
...
def main() -> None:
    ...
    with (args.output_dir / "figure_data_manifest_draft_v2_1.json").open("w", encoding="utf-8") as handle:
