...
sys.path.insert(0, "/root/autodl-tmp/GUOJI/Neural-GC")
sys.path.insert(0, "/root/autodl-tmp/GUOJI/baselines")
...
def load_nsvar_data(d=10, seed=0):
    p = f"/root/autodl-tmp/GUOJI/mamba_enhanced/data/nonstationary_var/num_nodes_{d}/true_lag_7/noise_scale_1/seed_{seed}"
    x = np.load(os.path.join(p, "_x.npy"))
    gc = np.load(os.path.join(p, "_gc.npy"))
    return x, gc
...
def run_tcdf_single(x, epochs=500, kernel_size=4, significance=0.8):
    ...
    import pandas as pd
    sys.path.insert(0, "/root/autodl-tmp/GUOJI/tcdf")
    import TCDF
