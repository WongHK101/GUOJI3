# Route-B Draft v2 Reproduction Instructions
...
```powershell
pdflatex -interaction=nonstopmode -halt-on-error istf_kbs_jacobian_coverage_draft_v2.tex
```

The expected outcome is a 12-page PDF with zero LaTeX errors, zero undefined
references/citations, zero missing files, and zero Overfull hboxes. Underfull
box warnings are documented in `DRAFT_V2_AUDIT_REPORT.md` and are visually
non-defective.
...
`figure_data_manifest_draft_v2.json`. The package already includes the exact
...
2. Verify figure inputs against `figures/coverage_audit_draft_v2/figure_data_manifest_draft_v2.json`.
3. Read `DRAFT_V2_EVIDENCE_TRACEABILITY.md` before interpreting any figure or
