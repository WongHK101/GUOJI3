# Route-B Draft v2.2 Evidence Traceability Register

## Scope

This register is the provenance companion for `istf_kbs_jacobian_coverage_draft_v2_2.tex`. It records the only empirical artifacts cited by the separate v2.2 draft. Hashes are recomputed when the review package is assembled. The canonical `E:\GUOJI\elsarticle\istf_kbs.tex` is not an input or output of this draft.
...
| Section 5.2 | Post-hoc mean $\lvert J_c\rvert/\lvert J_x\rvert=3.689\pm0.750$. | `E:\GUOJI\mamba_enhanced\risk_mitigation_results\concat_posthoc_jacobian.json`; generating script `experiments/risk_mitigation_20260515/run_concat_posthoc_jacobian.py`. | Full-input sensitivity-mass ratio, not a graph score or conditional-Granger estimate. | Controlled concat x-only MambaJRNGC; VAR(1), d=8, T=500, lag 1, d_cond=4, five saved seeds, 2,000 iterations. | Supporting five-seed post-hoc route-usage diagnostic only; not a benchmark, performance, or confirmatory result. |
