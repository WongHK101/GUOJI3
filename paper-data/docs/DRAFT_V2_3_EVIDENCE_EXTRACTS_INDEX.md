# Route-B Draft v2.3 Frozen Evidence Extracts

The review ZIP contains only frozen source files needed to inspect the figures
and tables cited in the separate v2.3 manuscript. These copies are not new
results and must not be modified, extended, or used to restart experimental
development. V2.3 changes scientific exposition and conceptual figure labels
only; its empirical figure inputs and numerical evidence are unchanged from
the approved V2.1 inventory.
...
| `frozen_evidence/concat_posthoc/concat_posthoc_jacobian.json` | `mamba_enhanced/risk_mitigation_results/concat_posthoc_jacobian.json` | Section 5.2; five-seed post-hoc sensitivity-mass route-usage diagnostic only, not a graph score, conditional-Granger estimate, benchmark result, performance result, or confirmatory result. |
