# Diagnostic Replication Audit: Route-B Draft v2.1
...
| Post-hoc auxiliary usage | Five saved seeds, summarized as mean +/- saved population SD. | Five-seed route-usage diagnostic, not a graph score. | Keep in prose only; do not promote it to graph-recovery evidence. |
...
- Diagnostic: a post-training full-input Jacobian is split into raw-history and auxiliary-coordinate blocks, then summarized as mean absolute sensitivity masses. The frozen mean ratio is $\lvert J_c\rvert/\lvert J_x\rvert=3.6890\pm0.7503$ (population SD).
- Manuscript status: supporting five-seed route-usage evidence only. It is not a graph score, a conditional-Granger estimate, or a performance comparison.
