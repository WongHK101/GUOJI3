...
def validate_criterion():
    ...
    print("\n--- Synthetic ---")
    for seed in range(5):
        path = os.path.join(our_data, "nonstationary_var", "num_nodes_10", "true_lag_7", "noise_scale_1", f"seed_{seed}")
    ...
    for seed in range(3):
        path = os.path.join(data_dir, "lorenz", "num_nodes_10", "F_40", f"seed_{seed}")
        try:
            x = np.load(os.path.join(path, "_x.npy"))
            r = recommend_filter(x, verbose=(seed == 0))
    ...
    for seed in range(3):
        path = os.path.join(data_dir, "var", "num_nodes_50", "true_lag_5", "noise_scale_1", f"seed_{seed}")
        try:
            x = np.load(os.path.join(path, "_x.npy"))
            r = recommend_filter(x, verbose=(seed == 0))
    ...
    # 4. NSVAR d=50 Plan A
    print("\n--- Plan A ---")
    plana_dir = os.path.join(our_data, "nsvar_plana")
    for seed in range(3):
        try:
    ...
    for subject in range(3):
        try:
            mat = scipy.io.loadmat(os.path.join(data_dir, "fMRI_15.mat"))
    ...
    for ds in ["medical", "pm25", "traffic"]:
        try:
            x = np.load(os.path.join(data_dir, "causaltime", ds, "_x.npy"))
