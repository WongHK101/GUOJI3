...
nsvar_aurocs = []
for seed in range(5):
    data_path = "/root/autodl-tmp/GUOJI/mamba_enhanced/data/nonstationary_var/num_nodes_10/true_lag_7/noise_scale_1"
...
nsvar50_aurocs = []
for seed in [0, 1, 2]:
    data_path = f"/root/autodl-tmp/GUOJI/mamba_enhanced/data/nonstationary_var_planA/num_nodes_50/true_lag_14/noise_scale_1/seed_{seed}"
    x = np.load(os.path.join(data_path, "_x.npy"))
    gc = np.load(os.path.join(data_path, "_gc.npy"))
    gc_pred = run_pcmci(x)
    met = compute_metrics(gc, gc_pred)
